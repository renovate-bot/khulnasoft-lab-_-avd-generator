...
func TrackerPostToMarkdown(tp TrackerPost, outputFile *os.File) error {
	t := template.Must(template.New("trackerPost").Parse(signaturePostTemplate))
...
func generateTrackerPages(rulesDir, postsDir string, clock Clock) {
	err := os.MkdirAll(postsDir, 0755)
	if err != nil {
		log.Fatal("unable to create tracker directory ", err)
	}

	log.Println("generating tracker pages in: ", postsDir)
...
func generateGoSigPages(rulesDir string, postsDir string, clock Clock) error {
	...
	for _, file := range files {
		func(file string) {
			...
			if findSubstringsInString(file, []string{"helpers.go", "example.go", "export.go", "trackerrego.go", "aio", "common", "mapper"}) || findSuffixSubstringInString(file, []string{".md", ".rego", "test.go", ".disabled"}) {
				return
			}
			...
			log.Printf("Processing Tracker go signature file: %s", file)
			...
			runTimeSecurityMenu.AddNode(topLevelID, strings.Title(topLevelIDName), postsDir, "", []string{"runtime"}, []menu.BreadCrumb{
				{Name: "Runtime Security", Url: "/tracker"},
			}, "runtime", true)
			parentID := topLevelID
			...
			if err != nil {
				log.Printf("unable to create tracker markdown file: %s for sig: %s, skipping...\n", err, sig.ID)
				return
			}

			if err = TrackerPostToMarkdown(TrackerPost{
				...
				log.Printf("unable to write tracker signature markdown: %s.md, err: %s", sig.ID, err)
				return
			}
		}(file)
	}
...
func generateRegoSigPages(rulesDir string, postsDir string, clock Clock) error {
	...
	for _, file := range files {
		...
		log.Printf("Processing Tracker rego signature file: %s", file)
		...
		topLevelID := strings.ToLower(strings.ReplaceAll(topLevelIDName, " ", "-"))
		runTimeSecurityMenu.AddNode(topLevelID, strings.Title(topLevelIDName), postsDir, "tracker", []string{"runtime"}, []menu.BreadCrumb{
		...
		if err = TrackerPostToMarkdown(TrackerPost{
			...
			log.Printf("unable to write tracker signature markdown: %s.md, err: %s", m.ID, err)
			continue
		}

		// TODO: Add MITRE classification details
		// TODO: Add ability to append custom khulnasoft blog post from another markdown
	}
	return nil
}
